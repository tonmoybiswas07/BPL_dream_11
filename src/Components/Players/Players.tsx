...
const Players = ({ playerPromise }: PlayersProps) => {
  ...
  return (
    <div>
      <div className="flex flex-col md:flex-row gap-5 md:gap-0 justify-between items-center container mx-auto my-7">
        ...
        <div>
          <button
            onClick={() => handleClickType("available")}
            className={`btn ${buttonType === "available" ? "btn-success" : ""} rounded-r-none`}
          >
            Available
          </button>
          <button
            onClick={() => handleClickType("selected")}
